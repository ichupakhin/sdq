...
/**
 * The JavaTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * JavaTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class JavaTaskItemDetector {
	...
	public List<org.emftext.language.java.resource.java.mopp.JavaTaskItem> findTaskItems(String text, int line, int charStart) {
		List<org.emftext.language.java.resource.java.mopp.JavaTaskItem> foundItems = new ArrayList<org.emftext.language.java.resource.java.mopp.JavaTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.java.resource.java.mopp.JavaTaskItem(keyword, message, localLine, offset, end));
