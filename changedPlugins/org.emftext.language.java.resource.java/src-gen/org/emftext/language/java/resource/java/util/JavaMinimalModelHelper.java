 ...
 */
public class JavaMinimalModelHelper {
	
	private final static org.emftext.language.java.resource.java.util.JavaEClassUtil eClassUtil = new org.emftext.language.java.resource.java.util.JavaEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.emftext.language.java.resource.java.util.JavaStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.emftext.language.java.resource.java.util.JavaListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.emftext.language.java.resource.java.util.JavaStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.emftext.language.java.resource.java.util.JavaListUtil.castListUnchecked(value);
