...
public class JavaPlugin extends Plugin {
	...
	public static final String PLUGIN_ID = "org.emftext.language.java.resource.java";
	/**
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, JavaPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, JavaPlugin.PLUGIN_ID, message);
		}
		final JavaPlugin pluginInstance = JavaPlugin.getDefault();
