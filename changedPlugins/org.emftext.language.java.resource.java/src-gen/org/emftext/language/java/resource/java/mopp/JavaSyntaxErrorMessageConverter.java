...
public class JavaSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public JavaSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.emftext.language.java.resource.java.mopp.JavaLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.emftext.language.java.resource.java.mopp.JavaLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.emftext.language.java.resource.java.mopp.JavaLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.emftext.language.java.resource.java.mopp.JavaLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.emftext.language.java.resource.java.mopp.JavaLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = org.emftext.language.java.resource.java.util.JavaStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
