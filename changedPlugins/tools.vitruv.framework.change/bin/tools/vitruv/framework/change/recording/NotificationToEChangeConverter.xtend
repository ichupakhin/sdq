...
final class NotificationToEChangeConverter {
	...
	private def handleResourceChange(NotificationInfo notification) {
		...
		switch (notification.getEventType()) {
			...
			case Notification.ADD_MANY: {
				//WORKAROUND: Added by Ilia Chupakhin.
				//Check if _newValue has the type List<EObject>
				//************************************************* 
				var _newValue = notification.getNewValue();
				if (!(_newValue instanceof List)) {
         		 	//java.lang.System.err.println("##################### _oldValue is NOT instanceof List ");
         			 return changes;
        		}

        		var List<?> tempList = ( _newValue as List<?>);
        
			    for (var int i = 0; i < tempList.size(); i++) {
			    	if(!(tempList.get(i) instanceof EObject)) {
			    		//java.lang.System.err.println("##################### tempList.get(i) is NOT instanceof EObject ");
			    		return changes;
			    	}
			    }
				//WORKAROUND end
			...
			case Notification.REMOVE_MANY: {
				...
				//Check if _newValue has the type List<EObject>
				//************************************************* 
				var _oldValue = notification.getOldValue();
				if (!(_oldValue instanceof List)) {
         		 	//java.lang.System.err.println("##################### _oldValue is NOT instanceof List ");
         			 return changes;
        		}

        		var List<?> tempList = ( _oldValue as List<?>);
